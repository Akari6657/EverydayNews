...
2. 不要轻易拆分同一冲突 / 同一人物 / 同一政策的不同报道角度，默认合并。
   仅在以下情况才拆开：
   - 经济/市场冲击（油价、航运、供应链）与核心政治/军事叙事明显分离，且有 ≥ 3 篇专门覆盖经济面
   - 某国国内政治反应是完全独立的新闻事件（如选举结果、议会弹劾），而不只是对外部事件的评论
   - 文章数超过 8 篇，且不同角度有完全不同的核心事实（例如”停火谈判”与”同期导弹袭击”并发）
   以上三条都不满足时，宁可把所有相关文章放入一个故事线。
...
THREAD_MERGE_PROMPT_TEMPLATE = """你是一位国际新闻编辑。下列故事线来自独立分批聚类，可能存在重复或高度相关的条目。

请找出描述同一核心事件的故事线，给出合并建议。

合并原则：
1. 仅合并明确描述同一核心事件（同一冲突/同一人物/同一政策）的故事线
2. 不同角度但同属一个事件的（如外交、军事、人道主义），建议合并
3. 经济/供应链影响若体量较大（article_count ≥ 3），可以保持独立
4. 确实不相关的故事线不要合并
...
{{
  "merges": [
    {{
      "ids": [1, 3],
      "topic": "合并后的中文主题",
      "topic_en": "Merged English Topic"
    }}
  ]
}}
- ids 中填写要合并的 thread_id（可以合并 2 个或更多）
- 未出现在任何 ids 中的故事线自动保持不变
- 如果没有需要合并的，返回 {{"merges": []}}
- 返回严格 JSON，不要输出任何解释、Markdown 或代码块
...
- importance: 0-10 的重要性评分，参考标准：
  - 9-10：全球重大事件，如领导人峰会、大规模军事冲突、金融系统性危机、重大自然灾害
  - 7-8：区域性重大事件，重要政策出台、大型选举结果、知名企业重大变化
  - 5-6：有实质价值的发展，值得关注但影响范围有限
  - 3-4：轻微更新，日常政治动态，较小规模事件
  - 0-2：无实质新闻价值，例行声明，软性内容
